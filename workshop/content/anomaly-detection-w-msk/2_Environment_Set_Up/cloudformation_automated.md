...
This will begin the process of deploy the required resources. You will automatically be redirected to the following screen
...
Cloud9 and MSK from a network point of view, must be able to communicate with each other. To achieve this we must update the network security group for MSK to accept incoming traffic from the Cloud9 security group.
...
3. Click on the **Inbound rules** tabe
...
5. Navigate to the Clou9 console
6. Click on **Open IDE**

![enviorment_set_up_msk_4](/images/anomaly-detection-w-msk/enviorment_set_up_msk_4.png)

7. In the Cloud9 enviorment navigate and open ```Kafka_OpenSearch_Anomaly_Detection/Kafka/1_create_topic.py``` 
...
The command will retrun a result *Created topic ApplicationMetricTopic*
...
3. In the Cloud9 enviorment navigate and open ```Kafka_OpenSearch_Anomaly_Detection/OpenSearch/1_create_index.py``` 
4. Replace the ```<opensearch_domain_endpoint_url>``` with the value of the **OSDomainURL** key from the CloudFormation stack output
6. Run the following command on the Cloud9 termainl ```cd ~```
7. Run the following command on the Cloud9 termainl ```pip install requests```
8. Run the python script by issueing the following comand in the Cloud9 terminal ```python environment/Kafka_OpenSearch_Anomaly_Detection/OpenSearch/1_create_index.py``` 
...
3. In the Cloud9 enviorment navigate and open ```Kafka_OpenSearch_Anomaly_Detection/OpenSearch/2_map_IAM_user.py```
...
6. Run the python script by issueing the following comand in the Cloud9 terminal ```python environment/Kafka_OpenSearch_Anomaly_Detection/OpenSearch/2_map_IAM_user.py```
